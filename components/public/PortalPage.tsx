...
type RejectionReason = 'duplicate' | 'low_score' | 'over_budget' | 'location'
...
function focusStyle(e: React.FocusEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) {
  e.target.style.borderColor = '#8CF000'
  e.target.style.boxShadow = '0 0 0 3px rgba(140,240,0,0.15)'
}
...
interface PortalPageProps {
  mode: 'vendor' | 'referral'
  token: string
}

export function PortalPage({ mode, token }: PortalPageProps) {
  ...
  const apiBase = mode === 'vendor' ? `/api/vendor-portal/${token}` : `/api/refer/${token}`
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Vendor portal: slots mandatory. Referral portal: optional.
    if (mode === 'vendor' && availabilitySlots.length === 0) {
  ...
  function backToList() { setState('list'); setSelectedPosition(null) }
  function submitAnother() { setState('list'); setSelectedPosition(null) }

  function toggleJD(id: string) {
    setExpandedJD(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n })
  }
  function togglePipeline(id: string) {
    setExpandedPipeline(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n })
  }

  const portalLabel = mode === 'vendor' ? 'Partner Portal' : 'Referral Portal'
  const submitBtnLabel = mode === 'vendor' ? 'Submit Candidate' : 'Refer a Candidate'
  const submitFormLabel = mode === 'vendor' ? 'Submit Candidate' : 'Refer a Candidate'
  const submitActionLabel = mode === 'vendor' ? 'Submit Candidate' : 'Submit Referral'
  const successTitle = mode === 'vendor' ? 'Candidate submitted!' : 'Referral submitted!'
  const successBody = mode === 'vendor'
    ? 'The candidate has been successfully added to the pipeline. Our team will review and be in touch.'
    : 'Thank you for the referral! Our team will review and be in touch.'
  const successAgain = mode === 'vendor' ? 'Submit another candidate' : 'Submit another referral'
      ...
      <div style={{ minHeight: '100vh', background: '#F5F0EB', fontFamily: 'Arial, "Open Sans", sans-serif' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 16px' }}>
          <div style={{ background: '#fff', borderRadius: 20, padding: '48px 40px', maxWidth: 440, width: '100%', textAlign: 'center', boxShadow: '0 2px 16px rgba(0,0,0,0.06)' }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#2F2C29', marginBottom: 10 }}>{successTitle}</h1>
            <p style={{ color: '#6b6560', fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>{successBody}</p>
            <button
  ...
  if (state === 'rejected') {
    const heading =
      ...
      rejectionReason === 'location' ? 'Location not eligible' :
      ...
      <div style={{ minHeight: '100vh', background: '#F5F0EB', fontFamily: 'Arial, "Open Sans", sans-serif' }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 16px 60px' }}>
          <div style={{ marginBottom: 24 }}>
            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9a9490', marginBottom: 6 }}>{portalLabel}</p>
          ...
          {/* Tabs — vendor mode shows My Submissions, referral mode does not */}
          <div style={{ display: 'flex', gap: 4, marginBottom: 24, background: '#e8e3dd', borderRadius: 12, padding: 4, width: 'fit-content' }}>
            <button style={tabStyle('positions')} onClick={() => setActiveTab('positions')}>
              Open Positions {openPositions.length > 0 && (
                <span style={{ marginLeft: 4, background: activeTab === 'positions' ? '#8CF000' : '#d5d0ca', color: activeTab === 'positions' ? '#000' : '#6b6560', borderRadius: 999, padding: '1px 7px', fontSize: 11 }}>{openPositions.length}</span>
              )}
            </button>
            {mode === 'vendor' && (
              <button style={tabStyle('submissions')} onClick={() => setActiveTab('submissions')}>
                My Submissions {submissions.length > 0 && (
                  <span style={{ marginLeft: 4, background: activeTab === 'submissions' ? '#8CF000' : '#d5d0ca', color: activeTab === 'submissions' ? '#000' : '#6b6560', borderRadius: 999, padding: '1px 7px', fontSize: 11 }}>{submissions.length}</span>
                )}
              </button>
            )}
          </div>
          ...
          {activeTab === 'positions' && (
            openPositions.length === 0 ? (
              <div style={{ background: '#fff', borderRadius: 20, padding: '40px', textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
                <p style={{ color: '#9a9490', fontSize: 14 }}>No open positions are currently available. Check back later or contact your Tenarai contact.</p>
              </div>
            ) : (
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                            {mode === 'vendor' && pos.budgetMonthly != null && (
          ...
          {/* ── My Submissions (vendor only) ── */}
          {mode === 'vendor' && activeTab === 'submissions' && (() => {
                ...
                <div style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
                  {filtered.length === 0 ? (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                      ...
                      <tbody>
                        {filtered.map((s, i) => (
                          <tr key={s.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f0ebe5' : 'none' }}>
                            <td style={{ padding: '14px 16px', fontWeight: 600, color: '#2F2C29' }}>{s.firstName} {s.lastName}</td>
    ...
    <div style={{ minHeight: '100vh', background: '#F5F0EB', fontFamily: 'Arial, "Open Sans", sans-serif' }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '40px 16px 60px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: '28px 36px', marginBottom: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9a9490', marginBottom: 8 }}>{submitFormLabel}</p>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ background: '#fff', borderRadius: 20, padding: '32px 36px', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
            ...
            {/* Referrer Name (referral mode only) */}
            {mode === 'referral' && (
              <div style={{ marginBottom: 18 }}>
                <InputField label="Your Name (Referrer)" required>
                  <input
                    type="text"
                    value={referrerName}
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              >
                {file ? (
                  ...
                  <>
                    <p style={{ fontSize: 14, color: '#9a9490' }}>Drag &amp; drop CV here, or <span style={{ color: '#2F2C29', fontWeight: 600 }}>Browse</span></p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
              <InputField label="First Name" required>
                <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} required style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
              </InputField>
              <InputField label="Last Name" required>
                <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} required style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
              </InputField>
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <InputField label="Email" required>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
              </InputField>
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <InputField label="Country" required>
                <select value={country} onChange={(e) => setCountry(e.target.value)} required style={{ ...inputStyle, cursor: 'pointer' }} onFocus={focusStyle} onBlur={blurStyle}>
            ...
            {/* Note — referral mode only */}
            {mode === 'referral' && (
              <div style={{ marginBottom: 14 }}>
                <InputField label="Note" hint="(optional)">
                  <textarea
                    value={note}
            ...
            <div style={{ marginTop: 24, borderTop: '1px solid #f0ebe5', paddingTop: 18 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4 }}>
                <h2 style={{ fontSize: 13, fontWeight: 700, color: '#2F2C29' }}>
                  Availability {mode === 'vendor' && <span style={{ color: '#8CF000' }}>*</span>}
                  {mode === 'referral' && <span style={{ color: '#9a9490', fontWeight: 400, fontSize: 11 }}> (optional)</span>}
                </h2>
                {selectedPosition && (
                  <span style={{ fontSize: 11, color: '#9a9490' }}>All times in {tzAbbr(selectedPosition.timezone)}</span>
                )}
              </div>
              <p style={{ fontSize: 12, color: '#9a9490', marginBottom: 14 }}>
                {mode === 'vendor'
                  ? "Share up to 5 time slots when you're available for an interview. This helps us schedule faster."
                  : "Optionally share time slots when the candidate is available for an interview."}
              </p>
              {availabilitySlots.map((slot, i) => {
                ...
                return (
                  <div key={i} style={{ marginBottom: 8 }}>
                    ...
                    {past && <p style={{ fontSize: 11, color: '#dc2626', marginTop: 3, marginLeft: 2 }}>Please select a future date and time.</p>}
                  </div>
                )
              })}
            ...
            <div style={{ marginTop: 20, borderTop: '1px solid #f0ebe5', paddingTop: 16 }}>
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
                ...
                />
                <span style={{ fontSize: 12, color: consentError ? '#e53e3e' : '#6b6560', lineHeight: 1.5 }}>
