...
export function GlobalInsights({ initial }: { initial: GlobalInsightsData | null }) {
  const [data, setData] = useState<GlobalInsightsData | null>(initial)
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/insights/generate-global', { method: 'POST' })
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      {notEnoughData && <p className="text-sm text-gray-400 italic">{notEnoughData}</p>}
      {error && <p className="text-sm text-red-600">{error}</p>}

      {!hasInsights && !notEnoughData && !error && (
        <p className="text-sm text-gray-400">
          Get AI-powered patterns from candidate feedback across all positions.
        </p>
      )}
      ...
      {hasInsights && (
        <div className="space-y-4">
          ...
          {data!.bottleneckPattern && (
            <div>
              <p className="text-xs font-medium text-gray-500 mb-1.5">Bottleneck Pattern</p>
