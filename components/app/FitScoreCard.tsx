  ...
  return (
    <div>
      ...
      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full ${score >= 85 ? 'bg-green-500' : score >= 70 ? 'bg-yellow-500' : score >= 50 ? 'bg-orange-500' : 'bg-red-500'}`}
          style={{ width: `${score}%` }}
        />
      </div>
    </div>
...
export function FitScoreCard({
  ...
  if (fitScore === null) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h2 className="text-sm font-medium text-gray-900 mb-3">Fit Score</h2>
  ...
  const scoredAgo = fitScoredAt
    ? `${Math.round((Date.now() - new Date(fitScoredAt).getTime()) / 60000)} min ago`
    : null
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
      ...
      {fitSummary && (
        <p className="text-sm text-gray-700">{fitSummary}</p>
      )}
